(function ( $ ) {
	...
	$.fn.mpmansory = function ( options ) {
		...
		$.fn.apply = function ( settings, nrOfColumns, wrappers, items ) {
			...
			var classStr = "col-" + settings.breakpoints.xs + " col-sm-" + settings.breakpoints.sm + " col-md-"+settings.breakpoints.md + " col-lg-" + settings.breakpoints.lg + " col-xl-" + settings.breakpoints.xl + " " + settings.columnClass;
		...
		$.fn.normalize = function() {
			...
			var cols = _this.children( '.'+settings.columnClass );

			if (cols.length > 0) {
				$.each(cols, function(){
					_this.prepend($(this).html());
					$(this).remove();
				});
			}
